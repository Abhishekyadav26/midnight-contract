...
// The generated contract wrapper (from `compact compile`) lives under
// `src/managed/wallet/contract/index.js`. We dynamically import it inside
// `main()` so `tsc` won't fail when those managed artifacts are not present.
import { CompiledContract } from "@midnight-ntwrk/compact-js";
import { deployContract } from "@midnight-ntwrk/midnight-js-contracts";
import { httpClientProofProvider } from "@midnight-ntwrk/midnight-js-http-client-proof-provider";
import { indexerPublicDataProvider } from "@midnight-ntwrk/midnight-js-indexer-public-data-provider";
// levelPrivateStateProvider is not required for this contract (no private state)
...
// Expect managed assets for the `wallet` contract under src/managed/wallet
const zkConfigPath = path.resolve(currentDir, "..", "src", "managed", "wallet");
const deploymentPath = path.resolve(currentDir, "..", "deployment-wallet.json");
...
// Types
// ---------------------------------------------------------------------------
// The wallet contract (wallet.compact) contains only public ledger state
// and a verifier circuit — no private state is expected here.
type WalletCircuits = ImpureCircuitId<any>;
...
async function main() {
  ...
  console.log("\n╔══════════════════════════════════════════════════════════╗");
  console.log("║           Wallet Contract Deploy Script                 ║");
  ...
  const zkConfigProvider = new NodeZkConfigProvider<WalletCircuits>(
    zkConfigPath
  );

  // This contract uses only public ledger state and a verifier circuit.
  // Omit privateStateProvider when the contract has no private state.
  const providers = {
  ...
  // --- Compile contract ---
  // Import the generated contract wrapper at runtime. If the managed assets
  // haven't been generated (via `npm run compile:wallet`), give a helpful
  // error message.
  let ContractCtor: any;
  try {
    // Use a computed file URL so TypeScript doesn't attempt to resolve the module
    // during type-checking. This works both from source and from `dist` after
    // postbuild copies managed assets.
    ...
    const fileUrl = (await import("url")).pathToFileURL(absPath).href;
    const mod = await import(fileUrl);
    ContractCtor = (mod as any).Contract ?? (mod as any).default ?? mod;
  } catch (e) {
    throw new Error(
      "Managed contract wrapper not found. Run `npm run compile:wallet` to generate src/managed/wallet"
    );
  }

  const walletCompiledContract = CompiledContract.make(
    "wallet",
    ContractCtor as any
  ).pipe(
  ...
  const contract = await withStatus(
    "Deploying wallet contract (this may take a few minutes)",
    async () => {
      // This contract has no private state, so only pass compiledContract.
      // Cast providers to any to satisfy the deployContract type overloads.
      return deployContract(
        providers as any,
        {
          compiledContract: walletCompiledContract
        } as any
      );
    }
  );
