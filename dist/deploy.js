...
async function main() {
    ...
    try {
        ...
        const absPath = path.resolve(currentDir, "..", "src", "managed", "wallet", "contract", "index.js");
        const fileUrl = (await import("url")).pathToFileURL(absPath).href;
        const mod = await import(fileUrl);
        ContractCtor = mod.Contract ?? mod.default ?? mod;
    }
    catch (e) {
        throw new Error("Managed contract wrapper not found. Run `npm run compile:wallet` to generate src/managed/wallet");
    }
    const walletCompiledContract = CompiledContract.make("wallet", ContractCtor).pipe(CompiledContract.withVacantWitnesses, CompiledContract.withCompiledFileAssets(zkConfigPath));
    // --- Deploy ---
    const contract = await withStatus("Deploying wallet contract (this may take a few minutes)", async () => {
